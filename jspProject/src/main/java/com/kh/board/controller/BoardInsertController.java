...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//enctype이 mutlipart/form-data로 잘 전송되었을 경우 전반적인 내용이 실행되도록 코드 작성
		if(ServletFileUpload.isMultipartContent(request)) {
			//파일업로드를 위한 라이브러리 : cos.jar()
			//www.servlets.com 다운로드 (COS File Upload Library - com.oreilly.servlet)
			 ...
			 * => 같은 파일명이 있을 경우 덮어씌어질 수 있고, 파일명에 한글/특수문자/공백(띄어쓰기)이 포함될 경우 서버에 따라 문제발생
			 * 
			 ...
			 * }
			 * 
			 * 나만의 방식대로 절대 겹치지 않도록 rename할 수 있도록 FileRenamePolicy클래스 만들기
