...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Get방식으로 요청 시 해당 doGet메소드가 자동으로 호출된다.
		// System.out.println("GET요청 정상적으로 받았습니다.");
		
		/**
		 * 첫번째 매개변수인 request에는 요청 시 전달된 내용들이 담겨있음(사용자가 입력한 값, 요청방식, 요청자의 ip주소 등)
		...
		*/ 
		...
		// 단점 : 불편, 복잡하다, 혹시라도 html을 수정한다면 Java코드를 수정하는 것이기 때문에 수정된 내용을 다시 반영시키고자
		//       서버를 재실행(restart) 해야된다.
		...
		// 3) 위에서 가져온 스트림을 통해서 응답html 구문을 한줄씩 출력
		out.println("<html>");
